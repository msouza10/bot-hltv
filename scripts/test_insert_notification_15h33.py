#!/usr/bin/env python3
"""
Script para inserir manualmente uma notificação com horário passado (15:33).
Testa se o sistema detecta e dispara notificações com scheduled_time no passado.
"""
...
async def insert_notification():
    """Insere uma notificação com scheduled_time = 15:33 (passado)"""
    
    cache_manager = MatchCacheManager(LIBSQL_URL, LIBSQL_AUTH_TOKEN)
    client = await cache_manager.get_client()
    
    logger.info("=" * 80)
    logger.info("🧪 TESTE: Inserindo notificação para 15:33 (5 minutos atrás)")
    logger.info("=" * 80)
    ...
    # 2. Calcular timestamp para 15:33
    logger.info("\n📍 PASSO 2: Calculando timestamp para 15:33...")
    target_time = datetime.now().replace(hour=15, minute=33, second=0, microsecond=0)
    target_time_str = target_time.isoformat()
    
    logger.info(f"✅ Timestamp alvo: {target_time_str}")
    logger.info(f"   (que foi há {(datetime.now() - target_time).total_seconds():.0f} segundos)")
    ...
    await client.execute(
        ...
        VALUES (?, ?, ?, ?, ?)
        """,
        [TESTING_GUILD_ID, match_id, 15, target_time_str, 0]
    )
    ...
    result = await client.execute(
        ...
        WHERE match_id = ? AND reminder_minutes_before = 15
        ORDER BY id DESC
        LIMIT 1
        """,
        [match_id]
    ...
    logger.info("   3. Você verá nos logs:")
    logger.info(f"      - 🚀 ENVIANDO AGORA: Match {match_id} | 15min antes")
    logger.info(f"      - [NOTIF-SUCCESS] ✅ ENVIADA COM SUCESSO!")
    logger.info("   4. A mensagem também aparecerá no Discord!")
    logger.info("\n💡 Se a notificação não for enviada em 2 minutos,")
    logger.info("   verifique os logs do bot para diagnosticar.\n")
